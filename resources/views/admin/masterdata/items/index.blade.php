...
@section('js')
    <script>
        function infoModal(id) {
            ...
            $.get("{{ url('/admin/masterdata/items/infomodal') }}/" + id, {}, function(data, status) {
                $('#loading-add').html(
                    '<button type="button" class="btn btn-primary me-3" id="add-btn" onclick="addItemsModal()">Add Items</button>'
                    )
                $('#kontennya').html(data)
