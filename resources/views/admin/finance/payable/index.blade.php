                ...
                <div class="card-header border-0">
                    <div class="card-title align-items-start flex-column">
                        <div class="d-flex align-items-center position-relative my-1">
                            <h5 class="fw-bolder text-gray-600">Hutang</h5>
                        </div>
                    </div>
                    ...
                    <div class="d-flex justify-content-end position-relative my-1">
                        <div class="form-group">
                            <div class="input-group">
                                ...
                                <input class="form-control" placeholder="Search" id="searchtablePayable" type="text">
                            </div>
                        </div>
                    </div>
                        ...
                        <table class="table table-striped table-row-bordered gy-5 gs-7 border rounded w-100"
                            ...
                            <thead>
                                <tr class="text-start text-gray-400 fw-bolder fs-7 text-uppercase gs-0">
                                    ...
                                    <th class="text-uppercase text-secondary text-md font-weight-bolder opacity-7">Nomor PO
                                    </th>
...
@section('js')
    <script>
        ...
        function info(id) {
            ...
            $.get("{{ url('/admin/finance/payable/info') }}/" + id, {}, function(data, status) {
                $('#indexContent').hide();
                $('#content').html(data)
                $('#content').show()
                $('#loading-add').html(
        ...
        function edit(id) {
            $('#loading-add').html(
                '<div class="spinner-grow text-success" role="status"><span class="sr-only"></span></div>')
            $.get("{{ url('/admin/finance/payable/edit') }}/" + id, {}, function(data, status) {
                $('#indexContent').hide();
                $('#content').html(data)
                $('#content').show()
                $('#loading-add').html(
        ...
        var tablePayable = $('#tablePayable').DataTable({
            ...
            processing: true,
            ajax: {
                url: "{{ url('/admin/finance/payable/list') }}"
            },
            columns: [{
                    data: 'DT_RowIndex',
                    searchable: false
        ...
        $('#searchtablePayable').keyup(function() {
            tablePayable.search($(this).val()).draw()
        });
